...
void getNeighborForkInfo(StackFrame &sf, vector<Instance>::iterator inst)
{
...
void getNeighborThreadInfo(StackFrame &sf, vector<Instance>::iterator inst)
{
  vector<Instance>::iterator minusOne = inst-1, plusOne = inst+1;
  //Check plusOne first
  if (equalInstance(*inst, *plusOne) &&
    ((*plusOne).frames)[sf.frameNumber].task_id >0)
    sf.task_id = ((*plusOne).frames)[sf.frameNumber].task_id;
  //Check minusOne then
  else if (equalInstance(*inst, *minusOne) &&
    ((*minusOne).frames)[sf.frameNumber].task_id >0)
        ...
        if ((*vec_sf_i).lineNumber > 0) {
          ...
          if (isForkStarWrapper((*vec_sf_i).frameName)) {      
            ...
            if ((*vec_sf_i).info.callerNode == -1) //since default is -1 if not changed
              getNeighborForkInfo(*vec_sf_i, vec_I_i);
            ...
            if ((*vec_sf_i).task_id == 0)
              getNeighborThreadInfo(*vec_sf_i, vec_I_i);
