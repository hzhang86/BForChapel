...
static uint16_t fork_serial_number = 0; // added by Hui for unique chpl_comm_fork* calls
__thread fork_t *param_plain;
__thread fork_t *param_nb;
__thread fork_t *param_large;
__thread fork_t *param_nb_large;
#define NUMFORK 16384 //should be enough for different (caller, callee, fid) on each node
typedef struct {
  int caller;
  int callee;
  chpl_fn_int_t fid;
} forkID_t;
// static variable will be automatically initilized to all '0'
static forkID_t forkInfoArray[NUMFORK];
static int zeroFork; //used for special case(0,0,0)
//
// Helper function to store unique forkID_t elements
//
static int findOldForkInfo(int caller, int callee, chpl_fn_int_t fid)
{
  ...
  for (i=0; i<NUMFORK; i++) {
    //it existed
    if (forkInfoArray[i].caller==caller && forkInfoArray[i].callee==callee
        && forkInfoArray[i].fid==fid) {
      return 1; 
    }
    //it did not exist, we've reached to the first empty element
    else if (!(forkInfoArray[i].caller || forkInfoArray[i].callee || forkInfoArray[i].fid)) {
      forkInfoArray[i].caller = caller;
      forkInfoArray[i].callee = callee;
      forkInfoArray[i].fid = fid;
      return 0;
    }
  }
...
/*  //This has been moved to chpl-fifo-gasnet.h
typedef struct {
...
static void fork_wrapper(fork_t *f) {
  param_plain = f;
  //volatile unsigned long param;
  //asm("\t movq %%rdi,%0" : "=r"(param));
  // Above added to store param into stack
  if (f->arg_size)
    chpl_ftable_call(f->fid, &f->arg);
  else
    chpl_ftable_call(f->fid, NULL);
...
void  chpl_comm_fork(c_nodeid_t node, c_sublocid_t subloc,
  ...
  // record the fork_serial_number and increment it
  f_num = fork_serial_number;
  fork_serial_number++;
      ...
      if (!findOldForkInfo(chpl_nodeID, node, fid)) {
        chpl_comm_cb_info_t cb_data = 
          {chpl_comm_cb_event_kind_fork, chpl_nodeID, node,
           .iu.fork={f_num, subloc, fid, arg, arg_size}};
        chpl_comm_do_callbacks (&cb_data);
        //cb_comm_fork(&cb_data);
      }
    }
...
void  chpl_comm_fork_nb(c_nodeid_t node, c_sublocid_t subloc,
  ...
  // Communications callback support: We hoist it from node != chpl_nodeID since
  // fork_nb_wrapper is also called for cases when node == chpl_nodeID; NOT ture for the other two
...
void  chpl_comm_fork_fast(c_nodeid_t node, c_sublocid_t subloc,
  ...
  if (chpl_nodeID == node) {
    ...
    if (passArg) {
      ...
      info->callee = node;    //added by Hui
      info->fork_num = f_num; //added by Hui
      info->caller = chpl_nodeID;
      info->subloc = subloc;
      info->ack = &done;
      info->serial_state = chpl_task_getSerial();
