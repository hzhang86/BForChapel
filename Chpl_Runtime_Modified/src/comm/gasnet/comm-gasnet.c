...
#include "chpl-fifo-gasnet.h" // factor out all typedefs from gasnet and fifo
...
__thread chpl_comm_on_bundle_t *param_plain;
...
static void fork_wrapper(chpl_comm_on_bundle_t *f) {
  // saved for fork stacktrace info Hui
  param_plain = f; 
  chpl_ftable_call(f->task_bundle.requested_fid, f);
...
void  execute_on_common(c_nodeid_t node, c_sublocid_t subloc,
                        ...
                        chpl_bool fast, chpl_bool blocking, uint16_t fork_num) {
  done_t done;
...
void  chpl_comm_execute_on(c_nodeid_t node, c_sublocid_t subloc,
  ...
  if (chpl_nodeID == node) {
    ...
    execute_on_common(node, subloc, fid, arg, arg_size,
                     /*fast*/ false, /*blocking*/ true, f_num);
  }
}
...
void  chpl_comm_execute_on_nb(c_nodeid_t node, c_sublocid_t subloc,
  ...
  if (chpl_nodeID == node) {
    ...
    execute_on_common(node, subloc, fid, arg, arg_size,
                      /*fast*/ false, /*blocking*/ false, f_num);
  }
}
