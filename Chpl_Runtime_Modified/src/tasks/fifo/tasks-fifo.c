...
static int zeroFid; 
// Helper function to store unique fid
static int findOldFid(chpl_fn_int_t fid)
{
  int i;
  if (fid == 0 && zeroFid == 0) {
    zeroFid = 1;
  ...
  for (i=0; i<NUMFID; i++) {
    ...
    //it did not exist=>we've reached to the first empty element
    else if (fidArray[i] == 0) {
      fidArray[i] = fid;
      return 0;
    }
  }
  // fidArray overflow!
  printf("Error: fidArray overflow!\n");
  return -1;
}
...
#ifdef ENABLE_OUTPUT_TO_FILE
      fprintf(pFile, "%lu ", (uint64_t)(task_pool_node->bundle.id));   
#endif
    }
...
*/
//--------^^^---special functions created for Sampling and task instrument---^^---------//
...
static void setup_main_thread_private_data(void)
{
  ...
  tp->ptask->next         = NULL;
  tp->ptask->prev         = NULL;
  // Initialize the field for Sampling purpose
...
// This creates a specific task for polling gasnet comm layer
...
thread_begin(void* ptask_void) {
  task_pool_p ptask;
  thread_private_data_t *tp;
  // To bookkeeping the ID of the task that's about to execute with thread_begin
  // Important to use "volatile" since otherwise it will be optimized out by comp
  ...
  if (getenv("ENABLE_SAMP") != NULL) {
    ...
    if (PAPI_overflow(tp->EventSet, PAPI_TOT_CYC, THRESHOLD, 
...
*/
    // tid saved for blame sampling
    task_pool_node = ptask;
    //reserved_taskID = (uint64_t)(ptask->bundle.requested_fid); //It's acutally the fid, but
                                                    //we keep the name for now
...
static void thread_end(void)
{
  ...
  tp = (thread_private_data_t*) chpl_thread_getPrivateData();
  if (tp != NULL) {
    // Stop PAPI here if the sampling was enabled
    if (getenv("ENABLE_SAMP") != NULL) {
...
task_pool_p add_to_task_pool(chpl_fn_int_t fid, chpl_fn_p fp,
  ...
  if (!findOldFid(ptask->bundle.requested_fid)) {
