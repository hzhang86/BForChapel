...
#define ENABLE_PRE
#ifdef USE_PAPI
#define UNW_LOCAL_ONLY //to speed up the calls of libunwind
#include <libunwind.h>
#include "papi.h"
#include <pthread.h>
#endif
...
#ifdef USE_PAPI 
void PAPIhandler_empty(int EventSet, void *address, long long overflow_vector, void *context);
static void enablePAPI(void);
static void getEarlyStackTrace(chpl_taskID_t newTaskID, int fromWhere, chpl_taskID_t minTID, chpl_taskID_t maxTID);//added 12/22/15: stacktrace before coforall
static void adjustMinMaxTID(chpl_taskID_t newTID, chpl_taskID_t *minTID, chpl_taskID_t *maxTID);
int THRESHOLD = 1073807359;//608888809;   // overflow threshold of PAPI_TOT_CYC event
//__thread int EventSet = PAPI_NULL; //Thread-local-storage variables, one for each thread
//__thread long long values; //TLS variable
//int EventSet = PAPI_NULL; //Thread-local-storage variables, one for each thread
//long long values; //TLS variable
static FILE *pFile;
static char buffer[50];
static chpl_thread_mutex_t stack_unwind_lock; // critical section lock
...
static void enablePAPI()
{
  int retval;
  fprintf(stderr, "In enablePAPI\n");
  ...
  if (PAPI_thread_init(pthread_self) != PAPI_OK)
    fprintf(stderr, "PAPI_thread_init() is not okay.\n");
  else
    fprintf(stderr, "PAPI_thread_init() success.\n");
}


static void getEarlyStackTrace(chpl_taskID_t newTaskID, int fromWhere, chpl_taskID_t minTID, chpl_taskID_t maxTID)
{
   
  unw_cursor_t cursor;
  unw_word_t ip, wordValue;
  unw_context_t uc;
  int count;
  char funcName[128];
  ...
  //fprintf(stderr, "getEarlyStackTrace called from %d\n", fromWhere); //For test
  // begin critical section
  chpl_thread_mutexLock(&stack_unwind_lock);

#ifdef ENABLE_OUTPUT_TO_FILE
  fprintf(pFile,"<----START preSpawn %lu %lu %lu\n", (uint64_t)newTaskID, (uint64_t)minTID, (uint64_t)maxTID);
#endif
  unw_getcontext (&uc);
  ...
  while (unw_step(&cursor)>0) {
    ...
    if(!funcName[0]) // sometimes libunwind just can't get the name ! DONTKNOWWHY
      strcpy(funcName,"***");
#ifdef ENABLE_OUTPUT_TO_FILE
	fprintf (pFile, "%d 0x%016lx %s ",count, (unsigned long)ip, funcName);
#endif
    ...
    if (strstr(funcName, "fork")!=NULL && strstr(funcName, "wrapper")!=NULL) {
      ...
      // the following offset was obtained by checking the assembly manually
      //if (strcmp(funcName, "fork_nb_wrapper") == 0)
      //  paramAddr = rbp-0x10;
      //else // fork_wrapper, fork_large_wrapper, fork_nb_large_wrapper
      //  paramAddr = rbp-0x20;

      //param = *((fork_t**)paramAddr);
...
static void adjustMinMaxTID(chpl_taskID_t newTID, chpl_taskID_t *minTID, chpl_taskID_t *maxTID)
{
  ...
  if (*minTID == 0 && *maxTID == 0) {
    ...
    *maxTID = newTID;
    ...
    if (newTID < *minTID)
      *minTID = newTID;
    if (newTID > *maxTID) //most happen here
      *maxTID = newTID;
  }
}
...
#ifdef USE_PAPI
  chpl_thread_mutexInit(&stack_unwind_lock);
  enablePAPI();
...
#ifdef USE_PAPI
    chpl_taskID_t minTID = 0, maxTID = 0;
#endif
    int task_cnt = 0;
...
#ifdef USE_PAPI
        adjustMinMaxTID(ltask->ptask->id, &minTID, &maxTID);
#endif
...
#ifdef USE_PAPI
#ifdef ENABLE_PRE
    getEarlyStackTrace(nested_task.id, 3, minTID, maxTID);
#endif
#endif
...
void chpl_task_startMovedTask(chpl_fn_p fp,
  ...
  *pmtwd = (movedTaskWrapperDesc_t)
  ...
  // added for the purpose of getEarlyStackTrace
  task_pool_p ptask = NULL;

  // begin critical section
  chpl_thread_mutexLock(&threading_lock);

  ptask = add_to_task_pool(movedTaskWrapper, pmtwd, pmtwd->chpl_data, NULL);
#ifdef USE_PAPI
#ifdef ENABLE_PRE
  getEarlyStackTrace(ptask->id, 1, ptask->id, ptask->id);
#endif
#endif
...
thread_begin(void* ptask_void) {
  ...
  thread_private_data_t *tp;
#ifdef USE_PAPI //IMPORTANT to use 'volatile', or it'll be optimized out
  volatile uint64_t reserved_taskID;
#endif
...
#ifdef USE_PAPI
    task_pool_node = ptask;
    reserved_taskID = (uint64_t)ptask->id; //for bookkeeping in sampling
#endif
...
static void thread_end(void)
{
  ...
  if (tp != NULL) {
#ifdef USE_PAPI
    int ret = PAPI_stop(tp->EventSet, NULL);
    if(ret != PAPI_OK)
      fprintf(stderr, "PAPI_stop() error: %s \n", PAPI_strerror(ret));
    else
      fprintf(stderr, "PAPI_stop() success.\n");
#endif
    if (tp->lockRprt != NULL) {
...
#ifdef USE_PAPI
#ifdef ENABLE_PRE
    getEarlyStackTrace(ptask->id, 2, ptask->id, ptask->id);
#endif
#endif
    //
...
//
static void
launch_next_task_in_new_thread(void) { //TOCHECK: 12/21/15 maybe unwind stack here?
  task_pool_p       ptask;
