...
//    fprintf(slurmFile, "cd $SBATCH_O_WORKDIR\n");
    fprintf(slurmFile, "%s/gasnetrun_ibv -n %d %s %s ", WRAP_TO_STR(LAUNCH_PATH), 
        numLocales, chpl_get_real_binary_wrapper(), chpl_get_real_binary_name());
    for (i=1; i<argc; i++) {
...
//  fprintf(expectFile, "-I %s ", slurmFilename);
    fprintf(expectFile, " %s/gasnetrun_ibv -n %d %s %s ", WRAP_TO_STR(LAUNCH_PATH),
        numLocales, chpl_get_real_binary_wrapper(), chpl_get_real_binary_name());
    for (i=1; i<argc; i++) {
