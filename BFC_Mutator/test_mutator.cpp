...
std::vector<BPatch_point *> *findEntryPoint(BPatch_addressSpace *app) {
	...
	appImage->findFunction("timeSuck", functions);
...
int binaryAnalysis(BPatch_addressSpace *app) {
	...
	appImage->findFunction("timeSuck", funcs);
...
int main() {
	char outFileName[] = "test_mutant";
	char *outFile = outFileName;

	const char *progName = "helloBlame_orig"; // = ...
	int progPID = 42; // = ...
	const char *progArgv[] = {"helloBlame_orig", "-h", NULL}; // = ...
