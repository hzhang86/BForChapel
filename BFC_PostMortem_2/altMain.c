	  ...
	  if (bm != NULL)
	      ...
	      if (bf)
		{
		  ...
		  if (bf->getBlamePoint() > 0)
		    {
		      //std::cout<<" Is BP,"<<bf->getBlamePoint()<<endl;
		      // frames(all frames), modules (hash of all modules), vec_SF_i (current iterator),
		      //    blamedParams(empty int vector), true (is a blamePoint), isBottomParsed(true/false)
		      bf->resolveLineNum(frames, modules, vec_SF_i, blamedParams, true, isBottomParsed, NULL, O);
		      return;
		    }
		  else
		    {
		      //std::cout<<" Is Not BP,"<<bf->getBlamePoint()<<endl;
