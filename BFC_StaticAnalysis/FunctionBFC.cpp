 ...
 *  
 *  
 *  Created by Nick Rutar on 5/14/09.
...
void FunctionBFC::LLVMtoOutputVar(VertexProps * v)
{
...
int FunctionBFC::checkForUnreadReturn(VertexProps * v, int v_index)
{
...
void FunctionBFC::goThroughAllAliases(VertexProps * oP, VertexProps * tP, std::set<VertexProps *> & visited)
{
...
void FunctionBFC::addControlFlowChildren(VertexProps * oP, VertexProps * tP)
{		
	VertexProps * tPSource = tP->dpUpPtr;
	if (tPSource == NULL)
		return;
...
void FunctionBFC::recursiveExamineChildren(VertexProps *v, VertexProps * origVP,
																						 std::set<int> & visited)
{
	...
	for(; e_beg != e_end; ++e_beg) 
	{
		VertexProps * targetVP = get(get(vertex_props, G), target(*e_beg, G));
		...
		if (opCode == Instruction::Store && (targetVP->storeLines.size() > 0 
																				 && in_deg != 1))
		{	
...
void FunctionBFC::resolveIVCalls()
{
	...
	set<VertexProps *>::iterator ivh_i;
	//set<VertexProps *> clearAfter;
	...
	for (ivh_i = impVertices.begin(); ivh_i != impVertices.end(); ivh_i++)
	{
		...
		if (ivp->nStatus[CALL_NODE])
		{
			...
			//for (ivp_i = ivp->clearAfter.begin(); ivp_i != ivp->clearAfter.end(); ivp_i++)
			//{}
		}
	}
}
...
void FunctionBFC::populateImportantVertices()
{
	...
	{
		VertexProps * targetVP = get(get(vertex_props, G),*i);
		//std::string na("GLOBAL");
		//ImpVertexProps * ivpParent = new ImpVertexProps(na);
...
FunctionBFC::~FunctionBFC()
{
	...
	//for (vec_ev_i = globalExitVariables.begin();  vec_ev_i != globalExitVariables.end(); vec_ev_i++)
	//delete (*vec_ev_i);
	...
	std::set<VertexProps *> vprops;
	graph_traits<MyGraphType>::vertex_iterator i, v_end;
  for(tie(i,v_end) = vertices(G); i != v_end; ++i) 
	{
		//int v_index = get(get(vertex_index, G),*i);
		VertexProps * v = get(get(vertex_props, G),*i);
		if (v == NULL)
			continue;
...
bool FunctionBFC::isTargetNode(VertexProps * ivp)
{
...
void FunctionBFC::makeNewTruncGraph()
{
  ...
  property_map<MyTruncGraphType, edge_iore_t>::type edge_type = get(edge_iore, G_trunc);
	...
	//ImpVertexHash::iterator ivh_i;	
	std::set<VertexProps *>::iterator ivh_i;
	...
	for (ivh_i = impVertices.begin(); ivh_i != impVertices.end(); ivh_i++)
	{
		...
		std::set<VertexProps *>::iterator ivp_i;
		std::set<VertexProps *>::iterator set_ivp_i;
		...
		{
			VertexProps * ivpParent = (*set_ivp_i);
		...
		/*
		 for (ivp_i = (*ivh_i).second->children.begin(); ivp_i != (*ivh_i).second->children.end(); ivp_i++)
		 {
		 ImpVertexProps * ivpChild = (*ivp_i);
		 tie(ed, inserted) = add_edge(ivpChild->number, ivp->number, G_trunc);
		 if (inserted)
		 edge_type[ed] = CHILD_EDGE;			
		 }
		 */
		...
		{
			VertexProps * ivpAlias = (*set_ivp_i);
	...
	//printFinalLineNums(std::cout);
}

void FunctionBFC::calcAggCallRecursive(VertexProps * ivp)
{
	...
	std::set<VertexProps *>::iterator v_vp_i;
...
void FunctionBFC::debugPrintLineNumbers(VertexProps * ivp, VertexProps * target, int locale)
{
...
void FunctionBFC::calcAggregateLNRecursive(VertexProps * ivp, std::set<VertexProps *> & vStack, 
																						 std::set<VertexProps *> & vRevisit)
{
	...
	std::set<VertexProps *>::iterator v_vp_i;
	std::set<VertexProps *>::iterator v_vp_i2;
	...
	for (v_vp_i = ivp->aliases.begin(); v_vp_i != ivp->aliases.end(); v_vp_i++)
	{
		...
		VertexProps * childFieldParent = child->fieldUpPtr;
		if (childFieldParent != NULL)
		{
			if (childFieldParent->name.find("parm") != std::string::npos)
			{
				ivp->descLineNumbers.insert(childFieldParent->externCallLineNumbers.begin(), 
																													childFieldParent->externCallLineNumbers.end());
...
void FunctionBFC::trimLocalVarPointers()
{
	...
	for (ivh_i = impVertices.begin(); ivh_i != impVertices.end(); ivh_i++)
	{
				...
				!ivp->nStatus[CALL_PARAM]  && !ivp->nStatus[CALL_RETURN])
		{
			...
			//VertexProps * ivpPT = ivp->pointsTo;;
			...
			/*
			 std::set<VertexProps *>::iterator set_vp_i;
			 for (set_vp_i = ivp->children.begin(); set_vp_i != ivp->children.end(); set_vp_i++)
			 ivp->pointsTo->children.insert(*set_vp_i);
			 */
...
int FunctionBFC::checkCompleteness()
{
		...
		if (ivp->eStatus > NO_EXIT || ivp->nStatus[EXIT_VAR_FIELD] )
		{
			...
			for(s_vp_i = ivp->descParams.begin(); s_vp_i != ivp->descParams.end(); s_vp_i++)
			{
				...
				for (v_ifc_i = param->calls.begin(); v_ifc_i != param->calls.end(); v_ifc_i++)
				{
					...
					VertexProps * cNode = ifc->callNode;
#ifdef DEBUG_COMPLETENESS								
					...
					for (v_ifc_i2 = cNode->calls.begin(); v_ifc_i2 != cNode->calls.end(); v_ifc_i2++)
					{
						...
						VertexProps * cNode2 = ifc2->callNode;
	...
	for (ivh_i = impVertices.begin(); ivh_i != impVertices.end(); ivh_i++)
	{
		...
		if (ivp->nStatus[CALL_NODE])
		{
			//if (secondCheck.count(ivp->line_num) == 0)
			//{
			secondCheck.insert(ivp->line_num);
			secondCheck.insert(ivp->descLineNumbers.begin(), ivp->descLineNumbers.end());
			std::set<ImpFuncCall *>::iterator v_ifc_i;
	...
	if (isMissing)
	{
		...
		set_difference(firstCheck.begin(), firstCheck.end(),
...
VertexProps * FunctionBFC::resolveSideEffectsCheckParentEV(VertexProps * vp, std::set<VertexProps *> & visited)
{
#ifdef DEBUG_SIDE_EFFECTS
...
VertexProps * FunctionBFC::resolveSideEffectsCheckParentLV(VertexProps * vp, std::set<VertexProps *> & visited)
{
#ifdef DEBUG_SIDE_EFFECTS
...
// written to as well
void FunctionBFC::resolveSideEffectsHelper(VertexProps * rootVP, VertexProps * vp, std::set<VertexProps *> & visited)
{
#ifdef DEBUG_SIDE_EFFECTS
...
#endif			
		std::set<VertexProps *>::iterator set_ivp_i2;
	...
	for (set_ivp_i = vp->children.begin(); set_ivp_i != vp->children.end(); set_ivp_i++)
	{
		...
		VertexProps * match = resolveSideEffectsCheckParentEV(*set_ivp_i, visited2);
...
void FunctionBFC::addSEAlias(VertexProps * source, VertexProps * target)
{
	std::vector< std::pair<VertexProps *, VertexProps *> >::iterator vec_pair_i;
	...
	{
		VertexProps * fir = (*vec_pair_i).first;
		VertexProps * sec = (*vec_pair_i).second;
	...
	std::pair<VertexProps *, VertexProps *> newOne(source, target);
	seAliases.push_back(newOne);
}


void VertexProps::getStructName(std::string & structName, std::set<VertexProps *> & visited)
{
...
std::string & VertexProps::getFullName()
{
	//std::cout<<"Entering getStructName"<<std::endl;
	//std::cout<<"Here "<<structName<<std::endl;
...
int VertexProps::getParamNum(std::set<VertexProps *> & visited)
{
	//std::cout<<"Entering getStructName"<<std::endl;
	//std::cout<<"Here "<<structName<<std::endl;
...
void FunctionBFC::addSERelation(VertexProps * source, VertexProps * target)
{
...
void FunctionBFC::recursiveSEAliasHelper(std::set<VertexProps *> & visited, VertexProps * orig, VertexProps * target)
{
	if (target == NULL)
		return;
		...
		if (ivp->eStatus >= EXIT_VAR_PARAM || ivp->nStatus[EXIT_VAR_FIELD] || ivp->nStatus[EXIT_VAR_PTR])
		{
			...
			for (vec_vp_i = ivp->aliases.begin(); vec_vp_i != ivp->aliases.end(); vec_vp_i++)
			{
				...
				else
				{
					...
					{
						//blame_info<<"IVP ExitV "<<ivp->exitV->name<<" target ExitV "<<(*vec_vp_i)->exitV->name<<std::endl;
						//if (ivp->exitV != (*vec_vp_i)->exitV)
							//addSEAlias(ivp, (*vec_vp_i));
						std::set<VertexProps *> visited;
						visited.insert(ivp);
...
void FunctionBFC::resolveSideEffectCalls()
{
	...
	for (ivh_i = impVertices.begin(); ivh_i != impVertices.end(); ivh_i++)
	{
		...
		if (ivp->nStatus[CALL_NODE])
		{
			...
			std::set<VertexProps *> parameters;
			std::set<VertexProps *> LVparameters;
			...
			for (v_ifc_i = ivp->calls.begin(); v_ifc_i != ivp->calls.end(); v_ifc_i++)
			{
				...
				VertexProps * cNode = ifc->callNode;
				std::set<VertexProps *> visited2;
				VertexProps * match = resolveSideEffectsCheckParentEV(cNode, visited2);	
				
				if (match != NULL)
				{
					pair<set<VertexProps *>::iterator,bool> ret;
					...
					if (ivp->sBFC == NULL /*&& ivp->nStatus[LOCAL_VAR] */)
					{
						...
						if (typeVal == Type::StructTyID)
						{
							...
							/*
							 ...
							 {
							 //std::string newFieldName = sb->structName;
							 //std::string extender("->");
							 //newFieldName += extender;
							 //newFieldName  += sf->fieldName;
...
void FunctionBFC::firstPass(Function *F, std::vector<VertexProps *> &globalVars,
		...
		std::ostream & call_file, int & numMissing) 
{
  ...
  genGraph(efInfo);
	...
	/*
	std::string vInfo("REGCAND/");
	vInfo += getModuleName();
	std::string plainDot(".");
	vInfo += plainDot;
	vInfo += getSourceFuncName();
	std::ofstream var_info;
	
	//std::cerr<<vInfo<<std::endl;
	var_info.open(vInfo.c_str());
	
	
	printFinalLineNums(var_info);
	*/
