...
class BlameFunction
{
 public:
  ...
  BlameFunction(std::string name){realName = name; blamePoint = NO_BLAME; isFull = false; 
								sideEffectC = false; hasSE = true; seAliasCalc = false;}
	BlameFunction(){blamePoint = NO_BLAME; isFull = false; sideEffectC = false; 
										hasSE = true; seAliasCalc = false;}
 	
	void addBlameToFieldParents(VertexProps * vp, std::set<VertexProps*> & blamees, short fromWhere);


  void resolveLineNum(vector<StackFrame> & frames, ModuleHash & modules,
			       vector<StackFrame>::iterator vec_SF_i, set<VertexProps *> & blamees,
											short isBlamePoint, bool isBottomParsed, BlameFunction * oldFunc, std::ostream &O);
											
	std::string getFullContextName(vector<StackFrame> & frames, ModuleHash & modules,
																	 vector<StackFrame>::iterator vec_SF_i);											

	void determineBlameHolders(std::set<VertexProps *> & blamees,
...
void addTempFieldBlameesRecursive(VertexProps * vp, VertexProps * blamee, std::set<VertexProps *> & oldBlamees,
  ...
  void calcAggregateLN_SE_Recursive(VertexProps * ivp);
	...
	bool transferFuncApplies(VertexProps * caller, std::set<VertexProps *> & oldBlamees,
																						VertexProps * callNode);
																						
	bool notARepeat(VertexProps * vp, std::set<VertexProps *> & blamees);

																						
	void outputFrameBlamees(std::set<VertexProps *> & blamees, std::set<VertexProps *> & localBlamees, 
  ...
  int getBLineNum() {return beginLineNum;}
  int setBLineNum(int bln) {beginLineNum = bln;}

  int getELineNum() {return endLineNum;}
  ...
  std::string getModulePathName() { return modulePathName;}
	...
	/////// ALIAS ONLY FUNCTIONS /////////////////
	BlameFunction * parseBlameFunction_OA(ifstream & bI);
	
	 void resolveLineNum_OA(vector<StackFrame> & frames, ModuleHash & modules,
			       vector<StackFrame>::iterator vec_SF_i, set<VertexProps *> & blamees,
											short isBlamePoint, bool isBottomParsed, BlameFunction * oldFunc, std::ostream &O);
											
	
    void determineBlameHolders_OA(std::set<VertexProps *> & blamees,
								std::set<VertexProps *> & oldBlamees,
								VertexProps * callNode,
								int lineNum, short isBlamePoint, std::set<VertexProps *> & localBlamees,
								std::set<VertexProps *> & DQblamees);						
	...
	void outputFrameBlamees_OA(std::set<VertexProps *> & blamees, std::set<VertexProps *> & localBlamees, 
											std::set<VertexProps *> &  DQBlamees, std::ostream &O);		
	...
	void addTempFieldBlameesRecursive_OA(VertexProps * vp, VertexProps * blamee, std::set<VertexProps *> & oldBlamees,
																			std::set<VertexProps *> & blamees, std::set<VertexProps *> & visited);	
																			
																													
	bool transferFuncApplies_OA(VertexProps * caller, std::set<VertexProps *> & oldBlamees,
																						VertexProps * callNode);																																															

	
	void addLoops_OA(ifstream & bI);
	float determineWeight(VertexProps * vp, int lineNum, std::set<VertexProps *> & blamees, bool skid);

	
	/// For Reads
	
	 void resolveLineNum_OAR(vector<StackFrame> & frames, ModuleHash & modules,
			       vector<StackFrame>::iterator vec_SF_i, set<VertexProps *> & blamees,
											short isBlamePoint, bool isBottomParsed, BlameFunction * oldFunc, std::ostream &O,
											bool skid);
	
											void	calcWeightForAll(std::set<VertexProps *> & blamees, int lineNum);
	
		float determineWeightSkid(VertexProps * vp, int lineNum, std::set<VertexProps *> & blamees);
									
	
    void determineBlameHolders_OAR(std::set<VertexProps *> & blamees,
								std::set<VertexProps *> & oldBlamees,
								VertexProps * callNode,
								int lineNum, short isBlamePoint, std::set<VertexProps *> & localBlamees,
								std::set<VertexProps *> & DQblamees, bool skid);			
								
	
	void skidDetermineBH_OAR(std::set<int> & lineNumbers,  std::set<VertexProps *> & blamees, float fVal);

	void skidItaniumDetermineBH_OAR(std::set<int> & lineNumbers,  std::set<VertexProps *> & blamees, 
				float fVal, int lineNum);

	int getNextValidLineNumber(std::set<int> & lineNumbers, int lineNum);
	int getPrevValidLineNumber(std::set<int> & lineNumbers, int lineNum);

																
	void outputFrameBlamees_OAR(std::set<VertexProps *> & blamees, std::set<VertexProps *> & localBlamees, 
											std::set<VertexProps *> &  DQBlamees, std::ostream &O);		
