...
Process::cb_ret_t on_signal(Event::const_ptr evptr)
{
  ...
  if (sigNo == 36) { //SIG36 is the overflow signal from PAPI 
    ...
    if (!ret) {
      ...
      inFrame.getName(fN1);
      cerr<<iter<<" "<<std::hex<<inFrame.getRA()<<std::dec<<" "<<fN1<<" ";
      while (ret) { 
        iter++;
        ret = walker->walkSingleFrame(inFrame, outFrame);
        outFrame.getName(fN2);
        if (ret) {
          cerr<<iter<<" "<<std::hex<<outFrame.getRA()<<std::dec<<" "<<fN2<<" ";
          inFrame = outFrame; 
        }
        else {
          cerr<<iter<<" Error "<<std::hex<<outFrame.getRA()<<std::dec<<" "<<fN2<<endl;
          break;
        }
      }
